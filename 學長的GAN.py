...
from sklearn.utils import resample, shufflesupport
...
class GAN():
    ...
    def load_predata(self, index):
        ...
        data = pd.read_csv("./data_10000/minmax-train - radom.csv")
        
        str_name = ['src_ip', 'dst_ip', 'server_port', 'proto']
    ...
    def train(self, epochs, batch_size=128, index=0):
        ...
        for epoch in range(epochs):
            ...
            if epoch == epochs-1:
                ...
                pd.concat([gloss, dloss], axis=1).to_csv('./gandata/loss(' + self.minor[index] + ').csv', index=False)
...
if __name__ == '__main__':
    ...
    def save_data():
        ...
        for i in range(len(g.rate)):
            ...
            data.to_csv('./gandata/data-gen(rate=' + str(g.rate[i]) + ').csv', index=False)
